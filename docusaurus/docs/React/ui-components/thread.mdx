...
smooth `Thread` mount and unmount behavior, wrap the main channel components in the [`Window`](../utility/window.mdx)
...
and components can be customized in a similar way. The UI components for both [`Message`](../ui/message-components/message.mdx)
and [`MessageInput`](../ui/message-input-components/message-input.mdx) can be overridden via props if you desire different
...
Additional props to be passed to the underlying [`MessageInput`](../ui/message-input-components/message-input.mdx) component.
...
Additional props to be passed to the underlying [`Message`](../ui/message-components/message.mdx) component, which represents the
...
Additional [props for `VirtualizedMessageList`](./virtualized-list.mdx#props) component.
...
Custom thread input UI component used to override the optional `Input` value stored in `ComponentContext` or the default. For the applications using [theme version 1](../../theming/v1/css-and-theming.mdx), the default is `MessageInputSmall`. Applications using [theme version 2](../../theming/v2/introduction.mdx) will use `MessageInputFlat` by default.

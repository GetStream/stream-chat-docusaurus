...
For a detailed example, review the [Message UI Customization](../../../customization-guides/message/message-ui.mdx) example.
...
Function that opens a [`Thread`](../../core/thread.mdx) on a message (overrides the function stored in `MessageContext`).

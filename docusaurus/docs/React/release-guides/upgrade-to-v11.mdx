...
And then you can pass these newly created options to [`Channel`](../components/core/channel.mdx) component which will be then propagated to `ReactionSelector` and `ReactionsList`:
...
The dependencies used to parse the markdown with [`renderText()` function](../components/core/message-list.mdx#rendering-message-text-with-rendertext-function) have been upgraded as a result of that, the following changes had to be performed in stream-chat-react library:

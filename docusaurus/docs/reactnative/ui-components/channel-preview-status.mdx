...
Component to render the status of the latest message on a channel within the [`ChannelList`](../core-components/channel-list.mdx).

## General Usage

You can customize this component and provide it back to the SDK via the [`PreviewStatus`](../core-components/channel-list.mdx#previewstatus) prop on `ChannelList` if desired.
